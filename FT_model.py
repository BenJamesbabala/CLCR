...
def attention_unet(input_shape, mask_channels, out_channels, multiplier, freeze_segmentor, use_constraints, dropout_rate):
    """
    ...
    mask_inputs = Input(shape=(input_shape[0], input_shape[1], mask_channels), name='input_masks')
...
def attention_unet_refined(input_shape, mask_channels, out_channels, multiplier, freeze_encoder, freeze_decoder, use_constraints, dropout_rate):
    """
    input_shape = (W, H) -- RGB Image
    out_channels = number of output segmentation masks
    multiplier = the scale by which the channels of the network can be increased
